...
class MuralEditar extends React.Component {
    ...
    componentDidMount = () => {
        ...
        if (lValid) {
            this.dbMuralRef = this.firebaseRef.child(`grupos/${grupoSelected.key}/mural`);
    ...
    componentWillUnmount = () => {
        if (this.dbMuralRef) this.dbMuralRef.off();
    }
    ...
    onConfirmRemove = (item) => {
        ...
        this.firebaseRef
        .child(`grupos/${grupoSelected.key}/mural/${item.key}`).remove()
        .then(() => showDropdownAlert('success', 'Publicação removida', ''))
        .catch(() => showDropdownAlert('error', ERROS.muralRemove.erro, ERROS.muralRemove.mes));
    }
