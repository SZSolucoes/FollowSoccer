...
class FinanceiroCadastrar extends React.Component {
    ...
    onPressConfirmar = () => {
        ...
        const dbAnaliseFina = databaseRef
        .child(`grupos/${grupoSelected.key}/financeiro/${dataKey}`);
        ...
        const asyncFun = async () => {
            ...
            if (snap && tipoOpe === 'receita' && snap.receita) {
                ...
                if (asyncAlert === 'sim') {
                    const dbAnaliseFinaRec = dbAnaliseFina
                    .child('receita');
                    ...
                    .then(
                        () => 
                        showDropdownAlert('success', 'Sucesso', 'Edição efetuada com sucesso')
                    )
                    .catch(() => showDropdownAlert(
                        'error', 
                        ERROS.financeiroEdit.erro, 
                        ERROS.financeiroEdit.mes
                    ));
                }
            ...
            // Senao existe entao verifica a inclusao
                ...
                .then(
                    () => 
                    showDropdownAlert('success', 'Sucesso', 'Inclusão efetuada com sucesso')
                )
                .catch(() => showDropdownAlert(
                    'error', 
                    ERROS.financeiroCad.erro, 
                    ERROS.financeiroCad.mes
                ));
            } else {
                const dbAnaliseFinaDes = dbAnaliseFina.child('despesa');
