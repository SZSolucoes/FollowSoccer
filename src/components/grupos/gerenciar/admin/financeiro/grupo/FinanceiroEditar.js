...
class FinanceiroEditar extends React.Component {
    ...
    onPressEditRemove = (item) => {
        const funExec = (itemSelected) => {
            const { grupoSelected } = this.props;

            const dbItemRef = this.dbFirebaseRef
            .child(`grupos/${grupoSelected.key}/financeiro/${itemSelected.key}`);

            dbItemRef.remove()
            .then(() => {
                setTimeout(
                    () => 
                    showDropdownAlert('success', 'Sucesso', 'Remoção efetuada com sucesso')
                , 1000);
            })
            .catch(() => 
                setTimeout(
                    ...
                    showDropdownAlert(
                        'error',
                        ERROS.financeiroRemove.erro,
                        ERROS.financeiroRemove.mes
                    )
                , 1000)
            );
        };
